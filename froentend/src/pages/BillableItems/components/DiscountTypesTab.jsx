...
const DiscountTypesTab = ({ isAddModalOpen, onCloseAddModal }) => {
    ...
    const fetchDiscounts = async () => {
        try {
            setIsLoading(true);
            const response = await settingsService.getAllDiscounts({ limit: 100 });
    ...
    const handleSave = async (data) => {
        try {
            if (editData) {
                await settingsService.updateDiscount(editData.id, data);
            } else {
                await settingsService.createDiscount(data);
            }
