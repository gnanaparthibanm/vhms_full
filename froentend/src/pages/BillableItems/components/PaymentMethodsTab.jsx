...
const PaymentMethodsTab = ({ isAddModalOpen, onCloseAddModal }) => {
    ...
    const fetchPaymentMethods = async () => {
        try {
            setIsLoading(true);
            const response = await settingsService.getAllPaymentMethods({ limit: 100 });
    ...
    const handleSave = async (data) => {
        try {
            if (editData) {
                await settingsService.updatePaymentMethod(editData.id, data);
            } else {
                await settingsService.createPaymentMethod(data);
            }
