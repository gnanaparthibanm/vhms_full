...
const TaxRatesTab = ({ isAddModalOpen, onCloseAddModal }) => {
    ...
    const fetchTaxRates = async () => {
        try {
            setIsLoading(true);
            setError(null);
            const response = await settingsService.getAllTaxRates({ limit: 100 });
            console.log('Tax Rates Response:', response);
            const data = response.data.data?.data || response.data;
            console.log('Extracted data:', data);
            const taxRatesArray = data.data || [];
            console.log('Tax Rates Array:', taxRatesArray);
            setTaxRates(taxRatesArray);
        } catch (err) {
    ...
    const handleSave = async (data) => {
        try {
            if (editData) {
                await settingsService.updateTaxRate(editData.id, data);
            } else {
                await settingsService.createTaxRate(data);
            }
            ...
            alert(err.message || 'Failed to save tax rate');
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            alert(err.message || 'Failed to delete tax rate');
        }
    };
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-[var(--border-color)]">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="p-8 text-center text-[var(--dashboard-text-light)]">
                                ...
                                taxRates.map((item) => (
                                    <tr key={item.id} className="group hover:bg-[var(--dashboard-secondary)] transition-colors">
                                        ...
                                        <td className="p-4">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item.is_default
                    ...
                    taxRates.map((item) => (
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span
                                    className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${item.is_default
                                            ? "bg-green-500/10 text-green-600 dark:text-green-400"
                                            : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400"
                                        }`}
                                >
