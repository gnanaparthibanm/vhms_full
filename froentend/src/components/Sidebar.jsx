...
const Sidebar = ({ isOpen, toggleMobileMenu, isCollapsed = false, setIsCollapsed }) => {
    ...
    // Helper to check if a sub-route is active for parent styling
    const isFinanceActive = ['/billable-items', '/bills-payments', '/inventory'].includes(location.pathname);
    ...
    const NavItem = ({ item, isSubItem = false }) => (
        <li>
            <NavLink
                ...
                className={({ isActive }) =>
                    `flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 group font-medium ${isActive
                        ? 'bg-[var(--sidebar-active-bg)] text-[var(--sidebar-active-text)] shadow-md'
                        : 'text-[var(--sidebar-inactive-text)] hover:bg-[var(--dashboard-secondary)]'
                    } ${isSubItem ? 'ml-8 text-sm' : ''} ${isCollapsed ? 'lg:justify-center lg:px-0 lg:w-12 lg:h-12 lg:mx-auto lg:rounded-xl' : ''}`
                }
            >
                <item.icon className={`w-4 h-4 ${isSubItem ? 'w-3 h-3' : ''}`} />
                <span className={`${isCollapsed ? 'lg:hidden' : ''} transition-opacity duration-100`}>{item.label}</span>
            </NavLink>
        </li>
    );

    return (
        <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] flex flex-col transition-all duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0 shadow-xl' : '-translate-x-full'} ${isCollapsed ? 'lg:w-20' : 'lg:w-64'}`}>
            <div className={`p-6 py-3 shadow-sm border-b border-[var(--border-color)] flex items-center ${isCollapsed ? 'lg:justify-center' : ''}`}>
                ...
                {!isCollapsed && (
                    <h1 className="text-xl font-semibold text-[var(--dashboard-primary)] px-2 whitespace-nowrap overflow-hidden transition-all duration-300">
                        AtelierVet
                    </h1>
                )}
            </div>
                ...
                <ul className="space-y-1">
                    ...
                    <li>
                        <button
                            ...
                            className={`w-full flex items-center ${isCollapsed ? 'lg:justify-center lg:px-0 lg:w-12 lg:h-12 lg:mx-auto lg:rounded-xl' : 'justify-between px-4'} py-3 rounded-xl transition-all duration-200 font-medium cursor-pointer group ${isFinanceActive && !financeOpen ? 'text-[var(--dashboard-primary)] bg-[var(--dashboard-secondary)]' : 'text-[var(--sidebar-inactive-text)] hover:bg-[var(--dashboard-secondary)]'
                                }`}
                        >
                            <div className="flex items-center gap-3">
                                <Wallet className="w-4 h-4" />
                                <span className={`${isCollapsed ? 'lg:hidden' : ''}`}>Finance</span>
                            </div>
                            <div className={`${isCollapsed ? 'lg:hidden' : 'block'}`}>
                                {financeOpen ? (
                                    <ChevronDown className="w-4 h-4 text-[var(--dashboard-text-light)]" />
                                ) : (
                                    <ChevronRight className="w-4 h-4 text-[var(--dashboard-text-light)]" />
                                )}
                            </div>
                        </button>

                        {(financeOpen) && (
                            <ul className={`mt-1 space-y-1 animate-in slide-in-from-top-2 duration-200 ${isCollapsed ? 'lg:hidden' : ''}`}>
            ...
            {/* User Profile Mini - Optional based on design, often at bottom of sidebar */}
            {/* 
      <div className="p-4 border-t border-gray-100">
          <div className="flex items-center gap-3 px-2">
             ...
             <div>
                 <p className="text-sm font-medium text-[var(--sidebar-inactive-text)]">Dr. Sarah Wilson</p>
